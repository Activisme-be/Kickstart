 ...
 * See usage in the Laravel framework site. 
 *
 * @author  	Tim Joosten
 * @package  	Blade
 * @category	Libraries
...
class Blade 
{
	...
	protected $_compilers = [
		'extensions', 'comments', 'echos', 'forelse', 'empty', 'endforelse',
		'structure_openings', 'structure_closings', 'else', 'unless', 'endunless',
        'includes', 'layouts', 'section_start', 'section_end', 'yields', 'yield_sections' 
	];
	...
	/**
	 * The setter for the Blade library. 
	 *
	 * @param  string $name
	 * @param  mixed  $value
	 * @return void
	...
	/**
	 * The unsetter for the Blade library. 
	 * 
	 * @param  string $name
	 * @return void
	 */
	...
	/**
	 * The getter for the Blade library.
	 *
	 * @param  string $name
	 * @return mixed
	 */ 
	...
	/**
	 * Adds a customs compiler function. 
	 *
	...
	/**
	 * Outputs template content. You can also pass array of global data. 
	...
	{
		// Default location. 
		$full_path = APPATH . 'views/' . $view . $this->blade_ext;

		// Modules Seperation / Modular extensions had been detected. 
	...
	protected function _compile($template)
	{
		...
		// Test if a compiles version exists in the cache. 
		if ($compiled = $this->cache->file->get($cache_id)) {
			if (ENVIRONMENT == 'production') { // In production, avoid to test if the template was updated. 
				return $compiled;
			}
	 ...
	 * @return string
	 */
	public function _run($template, $data = null) 
	{
		if (is_array($data)) {
			extract($data);
		}
	...
	/**
     * Get a template for use inside the current template. Inherits
